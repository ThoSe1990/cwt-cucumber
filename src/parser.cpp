...
std::pair<std::string, std::string> parse_keyword_and_name(lexer& lex,
                                                           bool remove_colon)
{
...
namespace
{
// Only used by the grammar-production helpers below, all of which live in
// this translation unit - kept file-local rather than declared in
// parser.hpp.
...
std::vector<std::string> doc_string_to_vector(const std::string_view s)
{
...
std::size_t advance_to_cell_end(lexer& lex)
{
  std::size_t count = 0;
...
cuke::value parse_cell(lexer& lex, bool remove_quotes_from_strings)
{
  token begin = lex.current();
...
cuke::value_array parse_row(lexer& lex, bool remove_quotes_from_strings)
{
  cuke::value_array v;
...
cuke::ast::example_node parse_example(lexer& lex,
                                      std::vector<std::string>&& tags)
{
  const std::size_t line = lex.current().line;
...
std::unique_ptr<cuke::ast::scenario_outline_node> make_scenario_outline(
...
std::unique_ptr<cuke::ast::scenario_node> make_scenario(
    lexer& lex, std::vector<std::string>&& tags,
    const std::optional<cuke::ast::rule_node>& rule,
...
std::optional<cuke::ast::rule_node> parse_rule(lexer& lex)
{
...
std::vector<std::unique_ptr<cuke::ast::node>> parse_scenarios(
...
std::unique_ptr<cuke::ast::background_node> parse_background(lexer& lex)
{
...
void parser::parse_from_file(const internal::feature_file& file)
{
