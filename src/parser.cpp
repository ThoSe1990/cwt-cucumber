...
[[nodiscard]] static std::vector<std::string> parse_description(
    lexer& lex, std::initializer_list<token_type> terminators)
{
...
[[nodiscard]] static std::optional<cuke::ast::rule_node> parse_rule(lexer& lex)
{
  if (lex.check(token_type::rule))
  {
    ...
    auto description = parse_description(
        lex, {token_type::scenario_outline, token_type::scenario,
...
const cuke::ast::gherkin_document& parser::head() const noexcept
{
  return m_head;
}
...
{
  m_error = false;
  internal::lexer lex(script, filename);
  lex.advance();
  lex.skip_linebreaks();
  m_head.set_feature(internal::parse_feature(lex));
  if (lex.error())
  {
