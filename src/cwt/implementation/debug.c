...
int disassemble_instruction(chunk* c, int offset)
{
  ...
  case OP_PRINT_STEP_RESULT:
    return constant_instruction("OP_PRINT_STEP_RESULT", c, offset);
  case OP_PRINT_BLACK:
  ...
  case OP_CALL_STEP_WITH_DOC_STRING:
    return constant_instruction("OP_CALL_STEP_WITH_DOC_STRING", c, offset);
