...
{
  auto pattern = create_regex_definition("{double}");
...
{
  {
    auto pattern = create_regex_definition("a step with a {string} inside");
    EXPECT_TRUE(step_finder("a step with a \"string value\" inside")
                    .step_matches(pattern));
  }
  {
    auto pattern = create_regex_definition("{string} at the beginning");
    EXPECT_TRUE(
        step_finder("\"string value\" at the beginning").step_matches(pattern));
  }
  {
    auto pattern = create_regex_definition("or a trailing {string}");
    EXPECT_TRUE(
        step_finder("or a trailing \"string value\"").step_matches(pattern));
  }
  {
    auto pattern = create_regex_definition(
...
{
  {
    auto pattern = create_regex_definition("a step with a {int} inside");
    EXPECT_TRUE(step_finder("a step with a 123 inside").step_matches(pattern));
  }
  {
    auto pattern = create_regex_definition("a step with a {double} inside");
    EXPECT_TRUE(
        step_finder("a step with a 123.456 inside").step_matches(pattern));
  }
  {
    auto pattern = create_regex_definition("{int} at the beginning");
    EXPECT_TRUE(step_finder("15 at the beginning").step_matches(pattern));
  }
  {
    auto pattern = create_regex_definition("or a trailing {double}");
...
{
  {
    auto pattern = create_regex_definition("a step with a {string}");
    EXPECT_FALSE(step_finder("a step with a 123").step_matches(pattern));
  }
  {
    auto pattern = create_regex_definition("a step with a {int}");
    EXPECT_FALSE(
...
{
  auto pattern = create_regex_definition("A box with {int} x {int} x {int}");
  EXPECT_TRUE(step_finder("A box with 2 x 2 x 2").step_matches(pattern));
}
TEST(step_finder, chinese_letters)
{
  auto pattern = create_regex_definition("一个盒子有 {int} x {int} x {int}");
...
TEST(step_finder, step_with_variables_1)
{
  ...
  auto pattern = create_regex_definition("A Step with a {string} and {int}");
...
{
  auto pattern = create_regex_definition("{int} a step");
...
{
  auto pattern = create_regex_definition("{double} a step");
...
{
  auto pattern = create_regex_definition("{float} a step");
...
{
  auto pattern = create_regex_definition("{string} a step");
...
{
  auto pattern = create_regex_definition("step with {word}");
...
{
  auto pattern = create_regex_definition("{word} in step");
...
{
  auto pattern = create_regex_definition("the {word} chapter is cool");
...
{
  auto pattern = create_regex_definition("it is {word} outside");
...
{
  auto pattern = create_regex_definition(".{word}.");
...
{
  auto pattern = create_regex_definition("two paraemters: {word} {int}");
...
TEST(step_finder, variable_and_word)
{
  ...
  auto pattern = create_regex_definition("A Step with a {word} and an {int}");
...
TEST(step_finder, variable_and_string)
{
  ...
  auto pattern = create_regex_definition("A Step with a {string} and an {int}");
  step_finder sf("A Step with a <value> and an 123", t.hash_row(1));
  ASSERT_TRUE(sf.step_matches(pattern));
  ASSERT_EQ(sf.values().size(), 2);
  EXPECT_EQ(sf.values()[0].as<std::string>(), std::string("some string value"));
...
TEST(step_finder, variable_and_byte)
{
  ...
  auto pattern = create_regex_definition("A Step with a {byte} and an {int}");
  step_finder sf("A Step with a <value> and an 123", t.hash_row(1));
  ASSERT_TRUE(sf.step_matches(pattern));
  ASSERT_EQ(sf.values().size(), 2);
...
TEST(step_finder, variable_and_short)
{
  ...
  step_finder sf("A Step with a <value> and an 123", t.hash_row(1));
  auto pattern = create_regex_definition("A Step with a {short} and an {int}");
  ASSERT_TRUE(sf.step_matches(pattern));
  ASSERT_EQ(sf.values().size(), 2);
  EXPECT_EQ(sf.values()[0].as<char>(), 99);
  EXPECT_EQ(sf.values()[1].as<int>(), 123);
...
TEST(step_finder, variable_and_int)
{
  ...
  auto pattern = create_regex_definition("A Step with a {int} and an {int}");
  step_finder sf("A Step with a <value> and an 123", t.hash_row(1));
  ASSERT_TRUE(sf.step_matches(pattern));
  ASSERT_EQ(sf.values().size(), 2);
...
TEST(step_finder, variable_and_long)
{
  ...
  auto pattern = create_regex_definition("A Step with a {long} and an {int}");
  step_finder sf("A Step with a <value> and an 123", t.hash_row(1));
  ASSERT_TRUE(sf.step_matches(pattern));
  ASSERT_EQ(sf.values().size(), 2);
...
{
  auto pattern = create_regex_definition("step with {}");
  step_finder sf("step with 15");
  ASSERT_TRUE(sf.step_matches(pattern));
  ASSERT_EQ(sf.values().size(), 1);
  EXPECT_EQ(sf.values().at(0).as<std::string>(), std::string("15"));
...
TEST(step_finder, anonymous_in_step_with_variable)
{
  ...
  auto pattern = create_regex_definition("A Step with a {} and an {int}");
  step_finder sf("A Step with a <value> and an 123", t.hash_row(1));
  ASSERT_TRUE(sf.step_matches(pattern));
  ASSERT_EQ(sf.values().size(), 2);
...
{
  auto pattern = create_regex_definition("step with {int} {} {word}");
...
{
  auto pattern = create_regex_definition("The box contains {int} item\\(s\\)");
