...
TEST(compiler, feature_chunk_code)
{
  ...
  EXPECT_EQ(feature->at(11), to_uint(hook_type::before));
  EXPECT_EQ(feature->at(12), 0);  // tag count (hook)

  EXPECT_EQ(feature->at(13), to_uint(op_code::constant));
  EXPECT_EQ(feature->at(14), 2);  // function: scenario call
  EXPECT_EQ(feature->at(15), to_uint(op_code::define_var));
  EXPECT_EQ(feature->at(16), 3);
  EXPECT_EQ(feature->at(17), to_uint(op_code::get_var));
  EXPECT_EQ(feature->at(18), 3);
  EXPECT_EQ(feature->at(19), to_uint(op_code::call));
  EXPECT_EQ(feature->at(20), 0);

  EXPECT_EQ(feature->at(21), to_uint(op_code::hook));
  EXPECT_EQ(feature->at(22), to_uint(hook_type::after));
  EXPECT_EQ(feature->at(23), 0);  // tag count (hook)

  EXPECT_EQ(feature->at(24), to_uint(op_code::scenario_result));
  EXPECT_EQ(feature->at(25), to_uint(op_code::func_return));
}
...
TEST(compiler, feature_constants)
{
  ...
  EXPECT_EQ(feature->constant(2).type(), value_type::function);
  EXPECT_EQ(feature->constant(2).as<function>()->name(), ":3");
  EXPECT_EQ(feature->constant(3).type(), value_type::string);
  EXPECT_EQ(feature->constant(3).as<std::string>(), ":3");
