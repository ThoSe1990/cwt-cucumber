...
- libFuzzer harnesses in `fuzz/` covering parser, scanner, step-definition matching, and placeholder substitution, with fixes for parser crashes and edge cases found during fuzzing ([141](https://github.com/ThoSe1990/cwt-cucumber/pull/141))
...
- Default assertion error messages no longer redundantly reference "in following step:" ([149](https://github.com/ThoSe1990/cwt-cucumber/pull/149))

### Fixed

- Assertions (e.g. `cuke::equal()`, `cuke::is_true()`) called inside scenario hooks (`BEFORE`/`AFTER`) crashed with empty steps or corrupted executed step status; hook assertions now route to `cuke::fail_scenario()` and preserve executed step results ([149](https://github.com/ThoSe1990/cwt-cucumber/pull/149))
